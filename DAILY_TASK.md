...
- `scripts/publish_daily.py` … 在庫から1本を `quotes.json` の先頭へ移し、`featuredDate` を当日にする。直近7本の人物とは重ならないよう選ぶ。公開済みと同じ・実質同じ（訳のゆれ程度）の格言は在庫から除去して公開しない。同日二重追加はしない。在庫が空ならスキップ（アプリは既存アーカイブから日替わり表示を継続）。
